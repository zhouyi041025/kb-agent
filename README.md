...
同一个 Key 还能把向量也换成真实语义向量，用来复测"混合检索到底能不能反超单路"：

```bash
export KB_EMBEDDING_PROVIDER=openai
export KB_EMBEDDING_MODEL=embedding-3
python scripts/build_index.py     # 换向量模型后必须重建索引
```
...
- **离线向量不理解同义词**：本仓库的检索结论只在这套词法设定下成立。配置真实语义向量后混合检索的收益需要重新测。
