...
企业知识库问答服务：**混合检索 + 可复现评测**。离线可跑，不需要 API Key。

- **可复现**：18 篇文档 / 50 条评测查询 / 完整消融脚本，`clone` 下来断网也能复现下面每个数字
- **检索链路**：查询改写 → BM25 + 向量双路召回 → RRF 融合 → 启发式重排 → 置信度门控（依据不足直接拒答）
- **拿得出手的结论**：混合 + 重排 R@1 **0.857**、MRR **0.906**；换成语义向量后混合检索反超单路（MRR **0.948**），而启发式重排的增益从 +0.119 变成 −0.024
- **工程化**：降级兜底 · 引用溯源 · LRU 缓存 · token 与成本核算 · Prometheus 指标 · 55 个单测 + CI
...
中文控制台：提问、查看召回片段与相关性分数、点引用跳回原文，检索方式 / top-k / 重排开关都能实时切。
截图是接上真实模型（免费档 `glm-4-flash`）的效果；下面评测用的是离线基线，见「接上真实模型之后」。
